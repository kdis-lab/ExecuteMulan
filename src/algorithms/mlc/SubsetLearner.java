...
public class SubsetLearner extends MultiLabelMetaLearner {
     ...
     * Constructor
     */
    public SubsetLearner(Classifier classifier) {
        this(new GreedyLabelClustering(new BinaryRelevance(classifier), classifier, new ConditionalDependenceIdentifier(classifier)), new LabelPowerset(classifier), classifier);
    }
    ...
    /**
     * Invokes the setSeed(seed) or setRandomSeed(seed) method of the supplied
     * object's Class, if such method exist.
     *
     * @param model which random seed should be reset.
     */
    public void resetRandomSeed(Object model, int seed) {
        Class aClass = model.getClass();
        Method method = null;
        try {
            method = aClass.getMethod("setSeed", int.class);
